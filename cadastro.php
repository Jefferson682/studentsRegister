    ...
    function listar($conexao){
        ...
        if($result = $conexao->query("select * from alunos")){
            while($row = $result->fetch_row()){
                ...
                $lista = $lista.'<button class="btn btn-green" type="button" onclick="editar('.$row[0].')">EDITAR</button>';
                $lista = $lista.'&nbsp;';
                $lista = $lista.'<button class="btn btn-red" type="button" onclick="excluir('.$row[0].')">EXCLUIR</button>';
                $lista = $lista.'</td>';
                $lista = $lista.'<tr>';
            }
        }else{
...
<body class="fundo-imagem">
        <form id="cadAluno" method="POST" action="cadastro.php">
            ...
            <button class="btn btn-green" type="button" onclick="salvar()">Salvar</button>
            <button class="btn btn-red" type="button" onclick="limpar()">Limpar</button> 
        </form>
        <p id="mensagem"></p>
        <table>
            <tr>
